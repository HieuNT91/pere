...
def _run_random(idx, seed_item_ids, item_embeddings, item_popularity, user_info, ec, method, logger):
    logger.info("Evaluating method %s with instance %d...", method, idx)
    step2 = RandomSequential(item_embeddings, max_questions=0, 
                            num_popular_items=ec.num_samples, 
...
def _run_dpp(idx, seed_item_ids, item_embeddings, item_popularity, user_info, ec, method, logger):
    logger.info("Evaluating method %s with instance %d...", method, idx)
    step2 = DPPSequential(item_embeddings, max_questions=0, 
                            num_popular_items=ec.num_popular_items, 
                            num_items_per_question=ec.num_items_per_question, 
                            user_info=user_info, item_popularity=item_popularity.copy(), 
                            logger=logger, gamma=0.5)
    step2.ask_and_update(seed_item_ids)
    phase2_item_ids = seed_item_ids.copy()
    num_likes = [len(step2.user_response['liked_list'])]
    num_dislikes = [len(step2.user_response['disliked_list'])]
...
def _run_greedy(idx, seed_item_ids, item_embeddings, item_popularity, user_info, ec, method, logger):
    logger.info("Evaluating method %s with instance %d...", method, idx)
    step2 = GreedySequential(item_embeddings, max_questions=0, 
                            num_popular_items=ec.num_popular_items, 
                            num_items_per_question=ec.num_items_per_question, 
                            user_info=user_info, item_popularity=item_popularity.copy(), 
                            logger=logger)
...
def _run_ours(idx, seed_item_ids, item_embeddings, item_popularity, user_info, ec, method, logger):
    logger.info("Evaluating method %s with instance %d...", method, idx)
...
def run_ept_phase2(ec, wdir, datasets, methods,
                    num_proc=4, start_seed=0, end_seed=5, logger=None):

    logger.info("Running ept phase 2...")
    if methods is None or len(methods) == 0:
        methods = ec.ephase2.all_methods
    if datasets is None or len(datasets) == 0:
        datasets = ec.ephase2.all_datasets 

    for dname in datasets:
        ret = dict()
        for mname in methods:
            for seed in range(start_seed, end_seed):
                res = run(ec.ephase2, num_proc, dname, mname, seed, logger)
                for k, v in res.items():
                    ret[k] = v
        helpers.pdump(ret, f'{dname}_{start_seed}_{end_seed}.pickle', wdir)

    logger.info("Done ept phase 2.")
