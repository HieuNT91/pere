...
def divide_category(info):
    keys = list(info.keys())
    for key in keys:
        val = info[key]
        val1 = val[:len(val)//3]
        val2 = val[len(val)//3:int(len(val) * 2/3)]
        val3 = val[:int(len(val) * 2/3)]
        info[key+"_1"] = [x for x in val1]
        info[key+"_2"] = [x for x in val2]
        info[key+"_3"] = [x for x in val3]
        del info[key]
    return info 
...
def purge_non_popular_item(info, ec):
    ...
    for key in keys:
        val = info[key]
        info[key] = [x for x in val if x < ec.num_popular_items]
    return info 
...
def run(ec, num_proc, dname, seed, logger):
    ...
    train_rating_dict = purge_non_popular_item(train_rating_dict, ec)
    category_to_items = purge_non_popular_item(category_to_items, ec)
    purge_dict(category_to_items)
    if dname == 'gowalla':
        category_to_items = divide_category(category_to_items)
    print(f'category has {len(category_to_items)} keys')
    purge_dict(category_to_items)

    def _run_dpe(user_info):
        logger.info("Running method dpe...")
        method = DPE(user_info, train_rating_dict, category_to_items)
    ...
    rets = joblib.Parallel(n_jobs=num_proc)(joblib.delayed(_run_dpe)(
    ...
    logger.info(f"Done dataset: {dname}, method: DPE!")
    return {"DPE" + f"_seed_{seed}": temp}


def run_ept_dpe(ec, wdir, datasets, num_proc=4, start_seed=0, end_seed=1, logger=None):

    logger.info("Running ept DPE...")
